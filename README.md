...
| `SCHEMA_NO_PARAMETERS` | MEDIUM | 零参工具，但 schema 没把 `additionalProperties` 关掉 |
...
| 6 | 输出产物写入失败（`--out` / `--sarif`）——**仅在没有更高优先级信号时** |

风险与漂移同时出现时返回 `4`——漂移是这个工具的核心信号。
**同时命中多条时按 `4 > 3 > 6` 取最前者**，所以"扫出 HIGH 风险"与"SARIF 没写出去"
一起发生时退出码是 **3** 而不是 6。`::error::` 注解不受这个优先级影响，三条都会打出来——
退出码只有一个，注解可以有好几行，按退出码分流与按注解分流的人应当看到同一件事。

> ⚠️ 上面这条优先级原先没写。实测（2026-09-22）：`--sarif` 写不出去而这次扫描有 HIGH 时，
> 退出码是 **3**——而表里 6 写的是"输出产物写入失败"，按 `if [ $? -eq 6 ]` 分支的脚本
> 会漏判"结果没上传"。这正是 README bug #14 记过的那个混淆（"扫描通过"与"结果上传了"
> 被当成同一件事）。四种情况会各自输出一行
...
20. **指纹变了却报「工具面与基线一致，无变化」，退出码 0。** 分级器只枚举它认识的
    那几类关键字，其余不产生 `Change`；而 `isClean()` 只看变更集是否为空——
    "分级器认不出"于是被当成了"没有变化"。实测四种改法（嵌套 object 里加参数、
    参数加 `const`、`items` 放宽类型、顶层加 `allOf`）**指纹全变了、变更集全是空的**。
    rug pull 的完整形态（"名字没变、参数多加一层"）只要写进已有 object 参数的嵌套里就报绿。
    修法是把不变量钉死：指纹变了 ⇒ 至少一条变更，认不出就记一条
    `UNGRADED_SCHEMA_CHANGE`（DANGEROUS）。**这是第 4 条（重名工具静默绕过）的同源路径**
21. **不可见字符集有两份，漂开了。** `RiskRules`/`TextNormalizer` 那份不含
    `U+2066–U+2069`，而 `Sanitizer` 另行把它们当不可见。于是
    `"Ig\u2066nore all previous instructions"` **退出码 0、零 finding**——
    既绕过 `INVISIBLE_CHARACTERS`（而规则表写着它管「双向控制字符」），
    又拆开了 `HIDDEN_INSTRUCTION` 的关键词。现有绕过用例只覆盖 `U+200B/U+200C` 那一档，
    所以这个洞看起来"已经被测过"。修法是字符集只剩一份，并加一条逐字符比对两份定义的测试
22. **零参工具被报 HIGH 并阻断 CI。** `SCHEMA_NO_PARAMETERS` 原先一律 HIGH：
    一个 14 个工具的良构文件系统工具面因此退出码 3，唯一那条 HIGH 落在
    `list_allowed_directories` 这类零参工具上。而它写的理由说的是**敞开**的 schema——
    实测 `additionalProperties: false`（最严）与 `true`（最松）报得一模一样，
    连 `type` 都不写的 `{}` 反倒不报：**惩罚显式声明、放过隐式开放**。
    现在显式关闭的零参 schema 一条都不报，其余降为 MEDIUM（默认阈值 HIGH，不再阻断）
23. **基线不可用没有 `::error::` 注解**，而它是 README 用整节论证的"最重要的失败模式"；
    按注解分流的人在 CI 上看不见它。**同一处还有一条优先级没写进退出码表**：
    风险（3）与产物写入失败（6）同时发生时退出码是 **3**，按 `[ $? -eq 6 ]` 分支的脚本
    会漏判"结果没上传"——正是第 14 条那个混淆的翻版
